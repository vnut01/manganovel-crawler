...
class NovelCrawler(BaseCrawler):
    def get_novel_info(self, url):
        ...
        if self.selectors["other_name"] != "":
            info["other_name"] = soup.select_one(
                self.selectors["other_name"]
            ).text.strip()
        if self.selectors["cover_image"] != "":
            info["cover_image"] = soup.select_one(self.selectors["cover_image"])[
                "data-src"
            ]
        ...
        if self.selectors["translator"] != "":
            info["translator"] = soup.select_one(
                self.selectors["translator"]
            ).text.strip()
        if self.selectors["status"] != "":
            info["status"] = soup.select_one(self.selectors["status"]).text.strip()
        if self.selectors["types"] != "":
            info["types"] = [
                a.text.strip() for a in soup.select(self.selectors["types"] + " a")
            ]
        if self.selectors["description"] != "":
            info["description"] = (
                soup.select_one(self.selectors["description"]).contents[0].strip()
            )
        if self.selectors["last_chapter_index"] != "":
            info["last_chapter_index"] = int(
                soup.select_one(self.selectors["last_chapter_index"]).text.strip()
            )
    ...
    def crawl_novel(self, url):
        ...
        chapters_need_download_index = list(
            range(
                novel["info"]["last_chapter_index_downloaded"] + 1,
                novel["info"]["last_chapter_index"] + 1,
            )
        )
        chap = self.fetch_chapter_content(url, chapters_need_download_index[0])
        if chap is not None:
            novel["chapters"][chapters_need_download_index[0]] = chap
            novel["info"]["last_chapter_index_downloaded"] = (
                chapters_need_download_index[0]
            )
